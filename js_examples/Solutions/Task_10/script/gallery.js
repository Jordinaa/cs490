...
Gallery.prototype.load_circles = function() {
    ...
    var move_album_photo=function(){gallery.move_album_photo.call(gallery,this);};
    $(this.circles_div).children().on("click",move_album);
    $(this.photos_div).children().on("click",move_album_photo);
...
Gallery.prototype.move_album_photo =function(photo) {
    var length= $(this.photos_div).children().length; //get the length of the photos
    var img_index=$(photo).index()+1; //get the next photo
    if(img_index>=length) //if it is the index of the last photo
        img_index=length-1; 
    var circle=$(this.circles_div).children().eq(img_index); //select the circle that corresponds to the photo
    this.move_album(circle);
};
